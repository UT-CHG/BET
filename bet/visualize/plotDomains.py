...
def show_param(samples, data, rho_D = None, p_true=None,
    ...
    scatter_2D(samples, sample_nos, rD, p_true, save, show,
...
def show_data(data, rho_D=None, Q_true=None, sample_nos=None,
    ...
    scatter_2D(data, sample_nos, rD, Q_true,
...
def show_data_domain_multi(samples, data, Q_true, Q_nums=None,
    ...
    for i in Q_nums:
        ...
        show_data_domain_2D(samples, data[:,[0, i]], Q_true[:,[0, i]],
            true_markers, true_colors, ylabel=r'$q_{'+str(i+1)+r'}$', triangles,
            save=True, show=False, filename)
...
def show_data_domain_2D(samples, data, Q_true, true_markers=['^', 's', 'o'],
    ...
    if trianges==None:
        ...
        triangles = triangulation.triangles
    if file_name==None:
        filename = ['domain_q1_q2_cs.eps', 'q1_q2_domain_Q_cs.eps']
    # Create figure
    plt.tricontourf(data[:,0],data[:,1], np.zeros((data.shape[0],)), triangles = triangles,
                colors = 'grey')
    plt.autoscale(tight=True)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.savefig(filename[0], bbox_inches='tight', transparent=True,
            pad_inches=0)
    # Add truth markers
    ...
    if save:
        plt.savefig(filename[1], bbox_inches='tight', transparent=True,
            pad_inches=0)
    if show:
        plt.show()
    else:
