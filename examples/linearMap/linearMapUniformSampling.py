...
Suggested changes for user:
    ...
    Also, we can do uniform random sampling with some simple tweaks.
    Consider defining a new variable as
    
        n_samples = 1E3
    
    and comment out the n0, n1, n2 terms and the samples=np.vstack(...
    command below under the "#set up samples" comment. Instead, use
    
        samples = calculateP.emulate_iid_lebesgue(lam_domain=lam_domain, num_l_emulate = n_samples)
...
Suggested changes for user:
    ...
    If using a regular grid of sampling (using the n0, n1, and n2 variables
    above to define samples), then set
    
        lambda_emulate = samples
    
    Otherwise, play around with num_l_emulate. A value of 1E2 will probably
    give poor results while results become fairly consistent with values that
    are approximately 10x the number of samples.
    ...
    being used, which in a measure-theoretic context implies that each Voronoi
    cell is assumed to have the same measure. This type of approximation is
    more reasonable for large n_samples due to the slow convergence rate of
