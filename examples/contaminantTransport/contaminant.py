...
labels = [
    'Source $y$ coordinate [L]',
    'Source $x$ coordinate [L]',
    'Dispersivity x [L]',
    'Flow Angle [degrees]',
    'Contaminant flux [M/T]']
...
output_samples.set_values(np.loadtxt(
    "files/data.txt.gz")[:, QoI_indices_observe])
...
# Choose unused QoI as prediction QoI and propagate measure onto predicted
# QoI data space
QoI_indices_predict = np.array([7])
output_samples_predict = samp.sample_set(QoI_indices_predict.size)
output_samples_predict.set_values(np.loadtxt(
    "files/data.txt.gz")[:, QoI_indices_predict])
