...
QoIs of size input_dim that produces the smallest measure of the inverse 
...
best_sets = cQoI.chooseOptQoIs_large(input_set, max_qois_return=5,
    num_optsets_return=2, inner_prod_tol=0.9, cond_tol=1E2, measure=True)

'''
We see here the expected measure ratios are small.  This number represents the
expected measure of the inverse image of a unit hypercube in the data space.
With the bin_size definition of the uncertainty in the data, here we expect to
see inverse solutions that have a smaller measure (expected measure ratio < 1)
than the original measure of the hypercube in the data space.

This interpretation of the expected measure ratios is only valid for inverting
from a data space that has the same dimensions as the paramter space.  When
inverting into a higher dimensional space, this expected measure ratio is the
expected measure of the cross section of the inverse solution.
'''
###############################################################################

# At this point we have determined the optimal set of QoIs to use in the inverse
# problem.  Now we compare the measure of the inverse solution using
...
# the measure of the inverse solution.  With the Monte Carlo assumption, this
# also tells us the approximate measure of this measure.
