...
class uniform_hyperrectangle_base(prob_uniform):
    ...
    def test_M(self):
        ...
        if not isinstance(self.center_pts_per_edge, collections.Iterable):
            ...
            assert len(self.rho_D_M) == np.sum(self.center_pts_per_edge+2)**self.mdim


class uniform_hyperrectangle_int(uniform_hyperrectanlge_base):
    """
    Set up :meth:`bet.calculateP.simpleFunP.uniform_hyperrectangle_user` on data domain.
    """
    def setUp(self):
        """
        Set up problem.
        ...
        super(uniform_hyperrectangle_base, self).setUp()        
