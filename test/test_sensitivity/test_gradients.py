...
class GradientsMethods:
    ...
    def test_sample_linf_ball(self):
        ...
        self.samples = grad.sample_linf_ball(self.centers,
            self.num_close, self.radius, self.lam_domain)
    ...
    def test_sample_l1_ball(self):
        ...
        self.tile = np.tile(self.centers, [1, self.num_close]).reshape(\
            self.num_centers*self.num_close, self.Lambda_dim)
        nptest.assert_array_less(np.linalg.norm(self.samples[self.num_centers:]\
            - self.tile, 1, axis=1), self.radius)
    ...
    def test_pick_ffd_points(self):
        ...
        nptest.assert_array_almost_equal(np.linalg.norm(self.centersrepeat - \
            self.samples[self.num_centers:], axis=1), self.radius * \
            np.ones(self.Lambda_dim * self.num_centers))

        # Test the method returns the correct number of samples
        self.assertEqual(self.samples.shape[0], (self.Lambda_dim+1) * \
            self.num_centers)
    ...
    def test_pick_cfd_points(self):
        ...
        nptest.assert_array_almost_equal(np.linalg.norm(self.centersrepeat - \
            self.samples[self.num_centers:], axis=1), self.radius * \
            np.ones(self.centersrepeat.shape[0]))
